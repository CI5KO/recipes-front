    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg p-6 shadow-lg mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Dificultad</p>
            <p className="text-lg font-bold">{recipe.difficulty}</p>
          </div>
          <div className="bg-blue-50 dark:bg-blue-900 rounded-lg p-4">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Porciones</p>
            <div className="flex items-center gap-2">
              <button 
                onClick={() => adjustServings(-1)}
                className="bg-blue-500 hover:bg-blue-600 text-white rounded p-1 transition"
              >
                <FiMinus size={16} />
              </button>
              <p className="text-lg font-bold text-blue-700 dark:text-blue-300">🍽️ {servings}</p>
              <button 
                onClick={() => adjustServings(1)}
                className="bg-blue-500 hover:bg-blue-600 text-white rounded p-1 transition"
              >
          ...
          <div className="bg-green-50 dark:bg-green-900 rounded-lg p-4">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Costo Total</p>
            <p className="text-lg font-bold text-green-700 dark:text-green-300">💵 ${totalCost.toFixed(2)}</p>
          </div>
          <div className="bg-green-50 dark:bg-green-900 rounded-lg p-4">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Por Porción</p>
            <p className="text-lg font-bold text-green-700 dark:text-green-300">${costPerServing.toFixed(2)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg p-6 shadow">
          ...
          <ul className="space-y-3">
            {recipe.recipeIngredients.map(ri => {
              ...
              const ingredientCost = ing ? adjustedQuantity * ing.pricePerUnit : 0;
              return (
                <li key={ri.id} className="flex justify-between items-start border-b border-gray-200 dark:border-gray-700 pb-3">
                  <div className="flex-1">
                    <p className="font-semibold text-gray-800 dark:text-gray-100">{ing?.name || "Desconocido"}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {adjustedQuantity.toFixed(2)} {ing?.unitOfMeasure}
                      {ri.notes && <span className="italic ml-1">({ri.notes})</span>}
                    </p>
                  </div>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg p-6 shadow">
          ...
          <p className="whitespace-pre-line text-gray-700 dark:text-gray-300 leading-relaxed">{recipe.instructions}</p>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg p-6 shadow">
          ...
          <div className="flex flex-wrap gap-2">
            {recipe.tags.map(tagId => {
              ...
              return tag ? (
                <div key={tag.id} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2">
